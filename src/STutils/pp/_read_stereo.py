...
def stereoseq(
    ...
    if dataset_id is None:
        ...
        gef_files = [
            filename
            for filename in os.listdir(dataset_id_path)
            if filename.endswith(SK.GEF_FILE)
        ]
    ...
    image_filenames = [
        i
        for i in os.listdir(path / SK.REGISTER)
        if any(pattern.match(i) for pattern in image_patterns)
    ]
    ...
    cellbin_gef_filename = [
        i
        for i in os.listdir(path / SK.CELLCUT)
        if any(pattern.match(i) for pattern in gef_patterns)
    ]
    squarebin_gef_filename = [
        i
        for i in os.listdir(path / SK.TISSUECUT)
        if any(pattern.match(i) for pattern in gef_patterns)
    ]

    table_pattern = re.compile(r".*" + re.escape(SK.CELL_CLUSTER_H5AD))
    table_filename = [
        i for i in os.listdir(path / SK.CELLCLUSTER) if table_pattern.match(i)
    ]
    ...
    images = {
        Path(name).stem: Image2DModel.parse(
            imread(path / SK.REGISTER / name, **imread_kwargs),
            dims=("c", "y", "x"),
            **image_models_kwargs,
        )
    ...
    if read_square_bin:
        ...
        for i in squarebin_gef[SK.GENE_EXP].keys():
            ...
            assert np.array_equal(
                df_gene[SK.OFFSET], np.insert(np.cumsum(df_gene[SK.COUNT]), 0, 0)[:-1]
            )
            ...
            df_points[SK.FEATURE_KEY] = [
                name
                for _, (name, cell_count) in df_gene[
                    [SK.FEATURE_KEY, SK.COUNT]
                ].iterrows()
            ...
            expression = coo_matrix(
                ...
                shape=(
                    len(points_coords),
                    len(df_points[SK.FEATURE_KEY].cat.categories),
                ),
            ).tocsr()
    ...
    for (x_index, x_row), (y_index, y_row) in tqdm(
        zip(x_coords.iterrows(), y_coords.iterrows(), strict=True),
