...
class SK(ModeEnum):
    ...
    REGISTER = "image"
    TISSUECUT = "image"
    SPATIALCLUSTER = "analysis"
    SATURATION = "analysis"
    CELLCUT = "feature_expression"
    CELLCLUSTER = "analysis"
    # file identifiers
    MASK_TIF = "_mask.tif"
    REGIST_TIF = "_regist.tif"
    TISSUE_TIF = "_tissue_cut.tif"
    CELL_CLUSTER_H5AD = "adjusted.h5ad"
    RAW_GEF = ".raw.gef"
    CELLBIN_GEF = ".adjusted.cellbin.gef"
    TISSUECUT_GEF = ".tissue.gef"
    TISSUE_GEM = "_raw_barcode_gene_exp.txt"
    # transcripts
    FEATURE_KEY = "gene"
    GENE_NAME = "geneName"
...
def stereoseq(
    ...
    """
    image_models_kwargs, _ = _initialize_raster_models_kwargs(
        image_models_kwargs, {})
    path = Path(path)

    if dataset_id is None:
        dataset_id_path = path / SK.COUNT_DIRECTORY
        gef_files = [filename for filename in os.listdir(
            dataset_id_path) if filename.endswith(SK.GEF_FILE)]
        ...
        if gef_files:
            ...
            raise ValueError(
                f"No {SK.GEF_FILE!r} files found in {dataset_id_path!r}")
    else:
    ...
    table_filename = [i for i in os.listdir(
        path / SK.CELLCLUSTER) if table_pattern.match(i)]
    ...
    obs = obs.drop([SK.DNBCOUNT, SK.CELL_AREA], axis=1)
    ...
    if read_square_bin:
        ...
        for i in squarebin_gef[SK.GENE_EXP].keys():
            bin_attrs = dict(
                squarebin_gef[SK.GENE_EXP][i][SK.EXPRESSION].attrs)
            ...
            df_gene = pd.DataFrame(
                arr, columns=[SK.FEATURE_KEY, SK.OFFSET, SK.COUNT])
            df_gene[SK.FEATURE_KEY] = df_gene[SK.FEATURE_KEY].str.decode(
                "utf-8")

            # create df for points model
            arr = squarebin_gef[SK.GENE_EXP][i][SK.EXPRESSION][:]
            df_points = pd.DataFrame(
                arr, columns=[SK.COORD_X, SK.COORD_Y, SK.COUNT])
            ...
            assert np.array_equal(df_gene[SK.OFFSET], np.insert(
                np.cumsum(df_gene[SK.COUNT]), 0, 0)[:-1])
            ...
            df_points[SK.FEATURE_KEY] = df_points[SK.FEATURE_KEY].astype(
                "category")
            ...
            obs = pd.DataFrame(
                {SK.INSTANCE_KEY: points_coords.index, SK.REGION_KEY: name_points_element})
            ...
            expression = coo_matrix(
                ...
                shape=(len(points_coords), len(
                    df_points[SK.FEATURE_KEY].cat.categories)),
            ).tocsr()
    ...
    cell_coordinates = [x for i in range(1, 33)
                        for x in ("x_" + str(i), "y_" + str(i))]
    ...
    df_coords = pd.DataFrame(
        new_arr, columns=cell_coordinates, index=cells_table.obs.index)
    ...
    sdata = SpatialData(images=images, tables=tables,
                        shapes=shapes, points=points)
    return sdata
