...
def cell_bin_plot(
    ...
    colorMap = {
        ...
        for connectedComponentsLabel, color in zip(
            res["connectedComponentsLabel"].values, res["color"].values, strict=True
        )
    }
    # 将背景（标签为0）的颜色设为黑色
    colorMap[0] = (0, 0, 0)
...
def plot_cellbin_gradient(
    ...
    Args:
        ...
        colors: color palette, defaults to "RdYlBu_r"
        dpi: dpi of output file, defaults to 600
        edge_cut: pixels to retain in edge cutting, defaults to 300
        add_scale_bar: add scale bar or not, defaults to True
        background: background color, defaults to "white"
        scale: scale bar length, defaults to 0.5
        length_fraction: scale bar length fraction, defaults to 0.25
        ax: matplotlib axes, defaults to None
        save: save figure or not, defaults to True
...
def plot_cellbin_discrete(
    ...
    Args:
        ...
        tag: tag name in adata.obs
        prefix: prefix of output file
        colors: color palette in getDefaultcolors, defaults to 9
        dpi: dpi of output file, defaults to 600
        edge_cut: pixels to retain in edge cutting, defaults to 300
        background: background color, white or black, defaults to "white"
        add_scale_bar: add scale bar or not, defaults to True
        scale: scale bar length, defaults to 0.5
        length_fraction: scale bar length fraction, defaults to 0.25
        ax: matplotlib axes, defaults to None
        add_legend: add legend or not, defaults to True
        save: save figure or not, defaults to True
